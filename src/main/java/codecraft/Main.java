...
public class Main {
	...
	public static void runCodecraft(String Username, String versionLabel) throws FileNotFoundException, Exception {
		...
		World.textures.init("Textures/TexturePack.bmp");
		World.skyBox1.init("Textures/skyBox6.bmp");
		World.skyBox2.init("Textures/skyBox5.bmp");
		World.skyBox3.init("Textures/skyBox5.bmp");
		World.skyBox4.init("Textures/skyBox5.bmp");
		World.skyBox5.init("Textures/skyBox5.bmp");
		World.skyBox6.init("Textures/skyBox5.bmp");
		Intro1.playIntro();
		new Thread() {
			public void run() {
		...
		while (!GLFW.glfwWindowShouldClose(WindowVariables.window) && !WindowVariables.Done()) {
			...
			while (true) {
				...
				try {
					Block b = World.getBlockAtPos(MathUtills.floor(x2 - Player.posX),
							MathUtills.floor(y1 - Player.posY + 1), +MathUtills.floor(z2 - Player.posZ));
				...
				try {
					Block b = World.getBlockAtPos(MathUtills.floor(x1 - Player.posX),
							MathUtills.floor(y2 - Player.posY + 1), +MathUtills.floor(z1 - Player.posZ));
					if (b != null) {
						by1 = y2;
						if (Player.hotBarItems[Player.hotBarIndex] == null) {
			...
			Block b = null;
			try {
				b = World.getBlockAtPos(MathUtills.floor(bx1 - Player.posX), MathUtills.floor(by1 - Player.posY + 1),
						+MathUtills.floor(bz1 - Player.posZ));
			} catch (Exception e) {
			...
			if (placeable) {
				...
				Block b6 = null;
				try {
					b1 = World.getBlockAtPos(MathUtills.floor(bx1 - Player.posX),
							MathUtills.floor(by1 - Player.posY + 1) - 1, +MathUtills.floor(bz1 - Player.posZ));
				} catch (Exception e) {

				}
				/*
				 * try { b2 = World.getBlockAtPos(MathUtills.floor(bx1 -Player.posX),
				 * MathUtills.floor(by1-Player.posY+1)+1,+MathUtills.floor(bz1-Player.posZ)); }
				 * catch(Exception e) {
				 * 
				 * } try { b3 = World.getBlockAtPos(MathUtills.floor(bx1 -Player.posX)+1,
				 * MathUtills.floor(by1-Player.posY+1),+MathUtills.floor(bz1-Player.posZ)); }
				 * catch(Exception e) {
				 * 
				 * } try { b4 = World.getBlockAtPos(MathUtills.floor(bx1 -Player.posX)-1,
				 * MathUtills.floor(by1-Player.posY+1)-1,+MathUtills.floor(bz1-Player.posZ)); }
				 * catch(Exception e) {
				 * 
				 * } try { b5 = World.getBlockAtPos(MathUtills.floor(bx1 -Player.posX),
				 * MathUtills.floor(by1-Player.posY+1),+MathUtills.floor(bz1-Player.posZ)+1); }
				 * catch(Exception e) {
				 * 
				 * } try { b6 = World.getBlockAtPos(MathUtills.floor(bx1 -Player.posX),
				 * MathUtills.floor(by1-Player.posY+1)-1,+MathUtills.floor(bz1-Player.posZ)-1);
				 * } catch(Exception e) {
				 * 
				 * }
				 */
				// if(b1 != null || b2 != null || b3 != null || b4 != null || b5 != null || b6
				// != null) {

				Player.blockSelected = new Vector3f(MathUtills.floor(x1 - Player.posX),
						MathUtills.floor(y1 - Player.posY + 1), +MathUtills.floor(z1 - Player.posZ));
				GL11.glColor4f(1, 1, 1, 0.25f);
				GL11.glEnable(GL11.GL_ALPHA_TEST);
				GL11.glAlphaFunc(GL11.GL_GREATER, 0.0f);
				GL11.glEnable(GL11.GL_BLEND);
				GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
				new BlockGrass(MathUtills.floor(x1 - Player.posX), MathUtills.floor(y1 - Player.posY + 1),
						+MathUtills.floor(z1 - Player.posZ), 0, 0).Draw();
			...
			/*
			 * d.x = Math.cos(Player.rotX)*Math.cos(Player.rotY); d.y =
			 * Math.sin(Player.rotX)*Math.cos(Player.rotY); d.z = Math.sin(Player.rotY);
			 * //System.out.println("x:"+d.x); // System.out.println("y:"+d.y);
			 * //System.out.println("z:"+d.z); PlotCell3f plotter = new PlotCell3f(0, 0, 0,
			 * 1, 1, 1); // From the center of the camera and its direction... plotter.plot(
			 * new Vector3f(Player.posX,Player.posY,Player.posZ), d, 1000); // Find the
			 * first non-air block while ( plotter.next() ) { Vector3i v = plotter.get();
			 * Block b = null; try { b = World.getBlockAtPos(-v.x, -v.y, -v.z);
			 * }catch(Exception e){
			 * 
			 * } System.out.println("x:"+v.x); System.out.println("y:"+v.y);
			 * System.out.println("z:"+v.z); if (b != null) { System.out.println("x:"+v.x);
			 * System.out.println("y:"+v.y); System.out.println("z:"+v.z); plotter.end(); //
